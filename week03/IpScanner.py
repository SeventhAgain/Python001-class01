...
# 解析 ip_range 放入 queue
def put_queue(queue: mp.Queue, ip_range, flag):
...
# ping 功能的实现
def process_ping(queue: mp.Queue, flag):
    ...
    while True:
        if queue.empty() and flag:
            ...
            logging.debug(
                f'Queue is empty currently, process {os.getpid()} will wait 1 seconds to retrieve data...')
            time.sleep(1)
        else:
            ip_addr = queue.get()
...
def scan_ip_range(max_processes, ip_range, result_queue):
    ...
    p = mp.Process(target=put_queue, args=(q, ip_range, flag))
    p.start()

    ping_process = None
    # 启动多线程 运行 process_ping 方法
    for c in range(max_processes):
        ping_process = mp.Process(target=process_ping, args=(q, flag))
